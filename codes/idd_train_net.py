...
def do_train(cfg, model, resume=False):
    ...
    with EventStorage(start_iter) as storage:
        for data, iteration in zip(data_loader, range(start_iter, max_iter)):
            ...
            if (
                cfg.TEST.EVAL_PERIOD > 0 and iteration >= 59190
...
def setup(args):
    ...
    cfg.SOLVER.MAX_ITER = 138110
    cfg.SOLVER.CHECKPOINT_PERIOD = 3946
    cfg.SOLVER.BASE_LR = 0.000925 # pick a good LR
    cfg.SOLVER.GAMMA = 0.67
    cfg.SOLVER.STEPS = (59190, 98650, )
    cfg.TEST.EVAL_PERIOD = 3946
...
def remove_solver_states(model_path):
    ...
    torch.save(model, '/ssd_scratch/cvit/myfolder/idd_data_coco/model_run1/model_final_weights.pth')
    print("Remove solver states complete")

def main(args):

    #Removing solver states from model_final which contains every checkpointable object
    #model_path = '/ssd_scratch/cvit/myfolder/idd_data_coco/model_run1/model_final.pth' #model_0078919.pth'
    #if comm.is_main_process():

...
def do_test(cfg, model):
    ...
    #results_i, valid_losses_reduced, valid_loss_dict_reduced = inference_on_dataset(model, data_loader, evaluator) # Validation losses
    print("Return from eval")
    
    #results[dataset_name] = results_i
    print("Do test before1 ", comm.get_rank())
    print("Do test before2 ", comm.get_local_rank())
    if comm.is_main_process():
         ...
         print("Inside if ", comm.get_rank(), comm.get_local_rank())

    print("Outside if ", comm.get_rank(), comm.get_local_rank())
    if len(results_i) == 1:
    ...
    with EventStorage(start_iter) as storage:
        for data, iteration in zip(data_loader, range(start_iter, max_iter)):
            ...
            if (
                ...
                print("After test ", comm.get_rank(), comm.get_local_rank())
                if comm.is_main_process():
            ...
            if iteration - start_iter > 5 and (iteration % 40 == 0 or iteration == max_iter):
...
def dict_to_xlsx(cmdict):
    ...
    wb = xw.Workbook('/ssd_scratch/cvit/myfolder/balloon/models/balloonscmstat.xlsx')
    wk = wb.add_worksheet()
    row = 0
    col = 0
...
def setup(args):
    ...
    for d in ["train", "val"]:
        DatasetCatalog.register("balloon_" + d, lambda d=d: get_balloon_dicts("/ssd_scratch/cvit/myfolder/balloon/" + d))
    ...
    cfg.OUTPUT_DIR = '/ssd_scratch/cvit/myfolder/balloon/models/'
    cfg.MODEL.WEIGHTS = "/ssd_scratch/cvit/myfolder/balloon/model_final_480dd8.pkl"

...
from detectron2.data.datasets import register_cityscape_instances
...
def do_test(cfg, model):
    ...
    if comm.is_main_process():
       if 'bboxcmdict' in results_i.keys():
...
def do_train(cfg, model, resume=False):
    ...
    default_validation_AP = 0
    default_validation_AP50 = 0
    best_model_dict = {}
    val_stat_cm_dict = {}
    ...
    if comm.is_main_process():
       model_cmrcnn_dict = {}
       model_cmrcnn_dict['model'] = model.state_dict()
       torch.save(model_cmrcnn_dict, "/ssd_scratch/cvit/myfolder/Model_CMRCNN_pretrained_COCO.pth")
    ...
    with EventStorage(start_iter) as storage:
        for data, iteration in zip(data_loader, range(start_iter, max_iter)):
            ...
            if (
                ...
                bboxcm_sw = False
                if 'bboxcmdict' in val_dict.keys():
                ...
                if comm.is_main_process():
                    ...
                    if ((val_dict['AP'] > default_validation_AP) and (val_dict['AP50'] > default_validation_AP50)):

                        default_validation_AP = val_dict['AP']
                        default_validation_AP50 = val_dict['AP50']
                        if bboxcm_sw == True:
            ...
            if iteration - start_iter > 5 and (iteration % 247 == 0 or iteration == max_iter):
...
def dict_to_xlsx(cmdict):
    ...
    wb = xw.Workbook('/ssd_scratch/cvit/myfolder/cityscapes/models/bestcmstat.xlsx')
    wk = wb.add_worksheet()
    row = 0
    col = 0
...
def setup(args):
    ...
    cfg.OUTPUT_DIR = '/ssd_scratch/cvit/myfolder/cityscapes/models/'
    ...
    cfg.DATASETS.TRAIN = ("cityscapes_fine_inst_seg_train",)
    cfg.DATASETS.TEST = ("cityscapes_fine_inst_seg_val",)
    cfg.SOLVER.BASE_LR = 0.001
    cfg.SOLVER.GAMMA = 0.75
    cfg.SOLVER.STEPS = (24700,)
    cfg.SOLVER.MAX_ITER = 29640 # 120 epochs approx.
    cfg.SOLVER.CHECKPOINT_PERIOD = 2470
    cfg.TEST.EVAL_PERIOD = 2470
    cfg.MODEL.WEIGHTS = "/ssd_scratch/cvit/myfolder/cityscapes/model_final_480dd8.pkl"
    #CMRCNN_model_surgery_coco_to_cityscapes.pth #CascadeRCNN model_final_480dd8.pkl #MaskRCNN model_final_af9cf5.pkl
    ...
    register_cityscape_instances("cityscapes_fine_inst_seg_train", {}, "/ssd_scratch/cvit/myfolder/cityscapes/leftImg8bit/train", 
                                                                          "/ssd_scratch/cvit/myfolder/cityscapes/gtFine/train", True, True)
    register_cityscape_instances("cityscapes_fine_inst_seg_val", {}, "/ssd_scratch/cvit/myfolder/cityscapes/leftImg8bit/val",
                                                                          "/ssd_scratch/cvit/myfolder/cityscapes/gtFine/val", True, True)
    return cfg
...
def remove_solver_states(model_path):
    ...
    torch.save(model, '/ssd_scratch/cvit/myfolder/cityscapes/models/model_final_weights.pth')
    print("Done removing solver states")

def main(args):

    #Removing solver states from model_final which contains every checkpointable object
    #model_path = '/ssd_scratch/cvit/myfolder/cityscapes/models_run2/model_dict_14825.pth'
    #if comm.is_main_process():
    ...
    do_train(cfg, model)
    model_path = '/ssd_scratch/cvit/myfolder/cityscapes/models/model_final.pth'
    if comm.is_main_process():
        remove_solver_states(model_path)
        print("Training Done")
